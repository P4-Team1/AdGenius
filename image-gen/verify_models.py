...
def test_flux_model(candidates):
    ...
    for model_id, desc in candidates:
        ...
        try:
            ...
            if "FP8" in desc:
                # 9B FP8 is a single safetensors file, so we use from_single_file
                # The repo ID is "black-forest-labs/FLUX.2-klein-9b-fp8"
                # The filename is likely "flux.2-klein-9b-fp8.safetensors" based on standard naming
                ckpt_id = "black-forest-labs/FLUX.2-klein-9b-fp8"
                filename = "flux.2-klein-9b-fp8.safetensors"
                
                print(f"Loading single file checkpoint from {ckpt_id}...")
                pipe = Flux2KleinPipeline.from_single_file(
                    f"https://huggingface.co/{ckpt_id}/blob/main/{filename}",
                    torch_dtype=torch.bfloat16, # Pipeline runs in bf16, weights are fp8
                    
                )
            else:
                # Standard folder-based models (4B, 9B BF16)
            ...
            pipe.to("cuda") 
            end_load = time.time()
            print(f"Model Load Time for {desc}: {end_load - start_load:.2f} seconds") 
...
def test_qwen_model():
    ...
    try:
        ...
        pipe.to("cuda")
        end_load = time.time()
        print(f"Model Load Time for Qwen: {end_load - start_load:.2f} seconds")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="all", choices=["flux-4b", "flux-9b", "flux-9b-fp8", "qwen", "all"], 
    ...
    if verify_environment():
        ...
        if args.model in ["flux-9b", "all"]:
            candidates = [("black-forest-labs/FLUX.2-klein-9B", "9B (High Quality - BF16)")]
            test_flux_model(candidates)

        # Flux 9B FP8 (Optimized)
        if args.model in ["flux-9b-fp8", "all"]:
            candidates = [("black-forest-labs/FLUX.2-klein-9b-fp8", "9B (FP8 Optimized)")]
            test_flux_model(candidates)
